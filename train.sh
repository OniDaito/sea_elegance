...
if [ -f $1/run.conf ]; then
  ...
  if [ $# != 3 ]
    then
      echo "Please pass in the directory to save results to, e.g train.sh <savedir> <datadir>"
      exit 1
  fi
  basedir=$1
  datadir=$2
  baseops="python train_unet.py --image-path ../wormz/dataset --train-size 220 --test-size 34 --epochs 100"
